...
logger = logging.getLogger('create_config_xcu')

extension_id = 'com.pwd.myextension'
xml_file = 'OptionsDialogDev.xcu'
...
def create_config_xcu():
    ...
    with open(xml_file, "w", encoding="utf-8") as f:
        ...
        root.append(Elem("node", {'oor:name': "Nodes"}))

        leaf = ElemLeaf({"Name": extension_id,
                         "Label": {"en-US": "Maximum Paper Size"}})
        name = "{}.Node1".format(extension_id)
        node = ElemNode({"Name": name,
                         "Label": {"en-US": "Extensions Example", },
                         "AllModules": "false"},
                        leaves=(leaf,))
        root[-1].append(node)
        #
